...
async function browserCheck(input) {
	...
	const logStream = fs.createWriteStream(
		path.join(
			...
			`${moment()
				.tz("Asia/Shanghai")
				.format("YYYY-MM-DD HH-mm-ss")}_Browser.log`
		),
		{ flags: "a" }
	);
	try {
		if (!input) {
			...
			if (site) {
				...
				console.log(
					chalk.red(`[${global.time()}] [BROWSER] [ERROR] 指定的 ID 不存在`)
				);
				logStream.write(
		...
		console.log(
			chalk.red(`[${global.time()}] [BROWSER] [ERROR] 发生错误：${error}`)
		);
		logStream.write(
		...
		console.log(
			chalk.cyan(`[${global.time()}] [BROWSER] [INFO] 检测完成 >> ${stats}`)
		);
		logStream.write(
...
async function check(driver, site, logStream) {
	try {
		await driver
			.manage()
			.setTimeouts({ pageLoad: process.env.LOAD_TIMEOUT * 1000 });
		...
		if (site.status.toString() >= 500) {
			...
			if (include) {
				...
				console.log(
					chalk.blue(
						`[${global.time()}] [BROWSER] [INFO] ID >> ${site.id}, Result >> ${
							site.status
						} → RUN`
					)
				);
		...
		console.log(
			chalk.blue(
				`[${global.time()}] [BROWSER] [INFO] ID >> ${
					site.id
				}, Result >> 不做修改, Reason >> ${error.message}`
			)
		);
		logStream.write(
			`\n[${global.time()}] [BROWSER] [INFO] ID >> ${
